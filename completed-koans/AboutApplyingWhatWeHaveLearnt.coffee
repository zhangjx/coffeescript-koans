...
describe 'About Applying What We Have Learnt', ->
  ...
  it 'should find needle in a haystack (functional)', ->
    ...
    findNeedle = (ops) ->
      even = (direction, distance) -> direction is 'FWD' and distance > 100
      if true in (true for x in ops when even x.direction, x.distance) then true else false
  ...
  it 'should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)', ->
    ...
    # filter functions
    total = 0
    even = (x) -> (x % 3 is 0) or (x % 5 is 0)
    total += i for i in [1..1000] when even i
